...
```{r lmer-geminate}
model.geminate <-  lmer(dur_vowel ~ dur_geminate*aspiration*syllables + dur_word +
                         (1|speaker) + (1|word),
                     data = icelandic.stops, REML = FALSE
                     )
...
```{r lmer-spread}
model.aspirated <- lmer(dur_voic ~ spread + syllables + vor + vowel +
                            (1|speaker) + (1|word),
...
```{r mixed-spread}
mixed(dur_voic ~ spread + syllables + vor + vowel +
                            (1|speaker) + (1|word),
                        data = icelandic.aspirated.stops
                        )
```

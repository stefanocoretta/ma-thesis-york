...
test <- wilcox.test(norm_abs_voic ~ syl, data = results)
...
According to a two-sample Mann-Whitney U-test, the mean duration of vowels in monosyllabic words (\Sexpr{round(mean(mono$norm_abs_voic),2)*100}\%) does not differ significantly from the mean duration in disyllabic words (\Sexpr{round(mean(di$norm_abs_voic),2)*100}\%) [U = 43349, p < 0.77].
...
The mean duration of vowels in milliseconds was \Sexpr{round(mean(mono_stop_nasp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(mono_stop_nasp$abs_voic),2)}) in monosyllabic words ending in a non-aspirated geminate stop, while it was \Sexpr{round(mean(mono_stop_asp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(mono_stop_asp$abs_voic),2)}) if followed by a pre-aspirated geminate stop.
\Cref{f:vowelduration} shows the ratio duration of the vowel in the two conditions.
The difference in the mean of the vowel ratio between the non-aspirated (\Sexpr{round(mean(mono_stop_nasp$norm_abs_voic)*100)}\%, s.d. = \Sexpr{round(sd(mono_stop_nasp$norm_abs_voic),2)}) and the aspirated class of words (\Sexpr{round(mean(mono_stop_asp$norm_abs_voic)*100)}\%, s.d. = \Sexpr{round(sd(mono_stop_asp$norm_abs_voic),2)}) was significant [U = 7793, p < 0.001].
...
In disyllabic words, vowels are \Sexpr{round(mean(di_stop_nasp$abs_voic),2)} (s.d. = \Sexpr{round(sd(di_stop_nasp$abs_voic),2)}) msec long if followed by a non-aspirated geminate, and \Sexpr{round(mean(di_stop_asp$abs_voic),2)} (s.d. = \Sexpr{round(sd(di_stop_asp$abs_voic),2)})msec long if followed by a pre-aspirated geminate.
In ratio terms, the vowel makes up the \Sexpr{round(mean(di_stop_nasp$norm_abs_voic)*100,2)}\% (s.d. = \Sexpr{round(sd(di_stop_nasp$norm_abs_voic),2)}) of the VOR in word with plain geminates, and the \Sexpr{round(mean(di_stop_asp$norm_abs_voic)*100,2)}\% (s.d. = \Sexpr{round(sd(di_stop_asp$abs_voic),2)}) in words with pre-aspirated geminates.
...
The ratio duration was, respectively, \Sexpr{round(mean(mono_nas_nasp$norm_abs_voic)*100)}\% (s.d. = \Sexpr{round(sd(mono_nas_nasp$norm_abs_voic),2)}) and \Sexpr{round(mean(mono_nas_asp$norm_abs_voic)*100)}\% (s.d. = \Sexpr{round(sd(mono_nas_asp$norm_abs_voic),2)}).
...
CVNCV words have a mean vowel duration of \Sexpr{round(mean(di_nas_nasp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(di_nas_nasp$abs_voic),2)}) if the nasal is voiced and \Sexpr{round(mean(di_nas_asp$abs_voic),2)} msec  (s.d. = \Sexpr{round(sd(mono_nas_asp$norm_abs_voic),2)}) if it is voiceless.
As percentages, vowels are \Sexpr{round(mean(di_nas_nasp$norm_abs_voic)*100)}\% of the VOR if followed by a voiced nasal, and \Sexpr{round(mean(di_nas_asp$norm_abs_voic)*100)}\% when the nasal is voiceless.
...
In the case of monosyllabic words ending in a /lC/ cluster, the mean duration of the vowels were \Sexpr{round(mean(mono_lat_nasp$abs_voic),2)} (s.d. = \Sexpr{round(sd(mono_lat_nasp$abs_voic),2)}) and \Sexpr{round(mean(mono_lat_asp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(mono_lat_asp$abs_voic),2)}) when followed, respectively by a voiced and voiceless lateral.
These correspond to the \Sexpr{round(mean(mono_lat_nasp$norm_abs_voic)*100)}\% and the \Sexpr{round(mean(mono_lat_asp$norm_abs_voic)*100)}\% of the VOR.
...
The duration of vowels in -VLCV words is on average \Sexpr{round(mean(di_lat_nasp$abs_voic),2)} msec long (s.d. = \Sexpr{round(sd(di_lat_nasp$abs_voic),2)}) when the lateral is voiced and \Sexpr{round(mean(di_lat_asp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(di_lat_asp$norm_abs_voic),2)}) when it is voiceless.
The vowel takes the \Sexpr{round(mean(di_lat_nasp$norm_abs_voic)*100)}\% of the VOR if followed by a voiced lateral, and \Sexpr{round(mean(di_lat_asp$norm_abs_voic)*100)}\% when the lateral is voiceless.
...
With words containing an /rC/ cluster, the mean duration of vowels is \Sexpr{round(mean(di_rho_nasp$abs_voic),2)} msec long (s.d. = \Sexpr{round(sd(di_rho_nasp$norm_abs_voic),2)}) when the rhotic is voiced and \Sexpr{round(mean(di_rho_asp$abs_voic),2)} msec (s.d. = \Sexpr{round(sd(di_rho_asp$norm_abs_voic),2)}) when it is voiceless.
The vowel takes the \Sexpr{round(mean(di_rho_nasp$norm_abs_voic)*100)}\% of the VOR if followed by a voiced rhotic, and \Sexpr{round(mean(di_rho_asp$norm_abs_voic)*100)}\% when the rhotic is voiceless.

...
According to a two-sample Mann-Whitney U-test, the mean duration of the vowel in monosyllabic words (\Sexpr{round(mean(mono$abs_voic),2)}) does not differ significantly from the mean duration in disyllabic words (\Sexpr{round(mean(di$abs_voic),2)}) [U = 35302, p < 0.001].

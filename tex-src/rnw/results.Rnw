...
<<vor, include=FALSE>>=
shapiro.test(mono_stop_asp$v_to_rel)
shapiro.test(mono_stop_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = mono_stop)
t.test(v_to_rel ~ asp, data = mono_stop)

shapiro.test(di_stop_asp$v_to_rel)
shapiro.test(di_stop_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = di_stop)
wilcox.test(v_to_rel ~ asp, data = di_stop)

shapiro.test(mono_nas_asp$v_to_rel)
shapiro.test(mono_nas_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = mono_nas)
t.test(v_to_rel ~ asp, data = mono_nas)

shapiro.test(di_nas_asp$v_to_rel)
shapiro.test(di_nas_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = di_nas)
t.test(v_to_rel ~ asp, data = di_nas)

shapiro.test(mono_lat_asp$v_to_rel)
shapiro.test(mono_lat_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = mono_lat)
t.test(v_to_rel ~ asp, data = mono_lat)

shapiro.test(di_lat_asp$v_to_rel)
shapiro.test(di_lat_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = di_lat)
t.test(v_to_rel ~ asp, data = di_lat)

shapiro.test(di_rho_asp$v_to_rel)
shapiro.test(di_rho_nasp$v_to_rel)
var.test(v_to_rel ~ asp, data = di_rho)
t.test(v_to_rel ~ asp, data = di_rho)
@
...
The mean duration of vowels in milliseconds was \Sexpr{round(mean(mono_stop_nasp$abs_voic),2)} in monosyllabic words ending in a non-aspirated geminate stop, while it was \Sexpr{round(mean(mono_stop_asp$abs_voic),2)} if followed by a pre-aspirated geminate stop.
...
The ratio duration of stop closure in words containing a sonorant + stop cluster was not significantly difference in the non-aspirated and aspirated condition, except in disyllabic words with a /lC/ [] and /rC/ cluster [].
Naturally, the duration of stop closure in words with geminate stops was significantly different, since closure in pre-aspirated stops 

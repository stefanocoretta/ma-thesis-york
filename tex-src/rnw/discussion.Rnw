...
Instead, vowels are the \Sexpr{round(mean(results_stop$norm_abs_voic[results_stop$asp == "yes"]),2) * 100}\% of the VOR if followed by pre-aspirated geminates and the \Sexpr{round(mean(results_stop$norm_abs_voic[results_stop$asp == "no"]),2) * 100}\%.
...
In the data available, the duration of the pre-aspirated geminates was on average longer (\Sexpr{round(mean(results_stop_asp$abs_clos),2)}) than the duration of the non-aspirated counterparts (\Sexpr{round(mean(results_stop_nasp$abs_clos),2)}).

...
%ADD many figures
%TODO compare with Ladefoged: he says glottal opening is timed the same and its same amplitude but is it true here?
...
The duration of voicing is, in fact, greater in words with pre-aspirated geminates than in words with plain geminates.
...
We could say that, even if some adjustments are required, the Icelandic data seem to indicate that the timing hypothesis is the right direction for further exploration.

...
lines(norm_abs_voic_mono_density, lw = 1)
lines(norm_abs_voic_bi_density, lw = 1, lty = 2)
...
shapiro.test(bi$norm_abs_voic)
@

A Wilcox test shows that the mean length of modal voicing is significantly longer in monosyllabic words (\Sexpr{round(mean(mono$norm_abs_voic), 2)}) than in bisyllabic words (\Sexpr{round(mean(bi$norm_abs_voic), 2)}).
Subsequent tests will be performed for monosyllabic and bisyllabic words separately.

<<>>=
wilcox.test(mono$norm_abs_voic, bi$norm_abs_voic)
@
...
\section{VOR}
<<>>=
boxplot(v_to_rel ~ asp, data = results)
shapiro.test(results$v_to_rel[results$asp == "yes"])
shapiro.test(results$v_to_rel[results$asp == "no"])
t.test(v_to_rel ~ asp, data = results)
@

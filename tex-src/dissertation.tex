...
\citet{maddieson1976} and \citet{maddieson1976a} found that, in Hindi and a few other Indic languages, vowels followed by aspirated consonants (like in the word \textit{sāt} ``seven'') are longer than vowel followed by non aspirated consonants (like in \textit{sāth} ``companionship'').
...
Similarly, voiced sonorants followed by a stop (/SC/) can contrast in the same contexts with voiceless sonorants followed by a stop (/S̥C/).
...
The vocal folds can either be stiffened so as to prevent them vibrating, or abducted just enough to increase the threshold of pressure build up and favour voicing.
...
For example: /kʰɔppɪ/ `young seal' [kʰɔppɪ], but /kɔppɪ/ `chamber pot (dative)' [kʰɔʰpɪ].
...
The methodologies of this research have gained the approval of the Ethics Committee and the subjects received an information sheet and they signed a consent form.
...
The informants received compensation for their time in the form of Amazon vouchers or chocolates for the value of £5.
...
The participant JR had to be excluded from the analysis since he misunderstood the task, while part of the participant SHG's task was lost due to technical fault in the recording equipment.
All the speakers were from the Southern parts of Iceland so they are assumed to speak the ``soft'' variety of Icelandic (see \Cref{s:icelandic}).
...
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
\includegraphics[width=0.8\textwidth]{img/mann-dur-box-1} 
...
Instead, laryngeal spreading starts earlier in words with pre-aspirated, while voicing is still active even after glottal spread is initiated (see \Cref{f:glottal}).
...
Knowing about tongue movements could make it easier to test for hypothesis that rely on articulatory factors.
...
Although this study focussed more on the phonetics of the aspiration effect, in \Cref{s:linguistic} I briefly discussed of its linguistic aspects.
...
%TODO last paragraph of conclusions
